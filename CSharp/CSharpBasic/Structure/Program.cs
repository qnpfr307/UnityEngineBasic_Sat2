...
// 구조체(Structure)
// 값 타입 (기본적으로는 힙 영역에 할당되지 않음)
// 클래스와 마찬가지로 멤버 변수, 함수 연산자 오버로딩 등을 할 수 있음.
//
// 언제 클래스 대신 구조체를 쓰는가?
// 값의 변화가 빈번하지 않게 일어날 때. &&
// 멤버 필드 크기의 합이 16 Byte 를 넘지 않을 때. &&
// 박싱이 거의 일어나지 않을 때. (박싱: 기본 타입을 object 타입으로 변환하는 과정. 언박싱: object 타입을 다른 자료형으로 변환하는 과정.) &&
// 기본적인 자료형들로 단일 값들을 나타낼 때. (int, float, double 등...)
